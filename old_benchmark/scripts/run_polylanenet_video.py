...
def load_config(config_path):
    ...
    settings.update(
        {
            "num_outputs": model_params.get("num_outputs", settings["num_outputs"]),
            "backbone": model_params.get("backbone", settings["backbone"]),
            "pred_category": model_params.get("pred_category", settings["pred_category"]),
            "curriculum_steps": model_params.get("curriculum_steps", settings["curriculum_steps"]),
            "img_size": dataset_params.get("img_size", settings["img_size"]),
...
class PolyLaneNetVideoRunner:
    def __init__(self, repo_path, weight_path, config_path=None, device="cuda", conf_threshold=None):
        ...
        self.input_h, self.input_w = self.settings["img_size"]
        ...
        self.model = PolyRegression(
            num_outputs=int(self.settings["num_outputs"]),
            backbone=self.settings["backbone"],
            pretrained=False,
            pred_category=bool(self.settings["pred_category"]),
            curriculum_steps=self.settings["curriculum_steps"],
        ).to(self.device)
        ...
        if load_result.missing_keys or load_result.unexpected_keys:
            raise RuntimeError(
                "Checkpoint does not match the PolyLaneNet model. "
    ...
    def infer(self, frame, video_id, frame_id):
        ...
        lanes = self._decode_lanes(outputs, image_width, image_height)
        ...
        return LaneFrameResult(
            video_id=video_id,
            frame_id=frame_id,
            method="PolyLaneNet-ResNet50-TuSimple",
            lanes=lanes,
            runtime=RuntimeInfo(
    ...
    def _decode_lanes(self, outputs, image_width, image_height, points_per_lane=80):
        decoded = outputs.detach().cpu().numpy().reshape(-1, 7)
        decoded[:, 0] = sigmoid(decoded[:, 0])

        lanes = []
        for lane_index, raw_lane in enumerate(decoded):
            confidence = float(raw_lane[0])
            if confidence < self.conf_threshold:
                continue

            y_start = float(np.clip(raw_lane[1], 0.0, 1.0))
            y_end = float(np.clip(raw_lane[2], 0.0, 1.0))
            if abs(y_end - y_start) < 1e-4:
                continue

            ys = np.linspace(y_start, y_end, num=points_per_lane, dtype=np.float32)
            coeffs = raw_lane[3:]
            xs = np.polyval(coeffs, ys)
            valid = (xs >= 0.0) & (xs <= 1.0) & (ys >= 0.0) & (ys <= 1.0)
            xs = xs[valid]
            ys = ys[valid]
            if len(xs) < 2:
                continue
...
def run_video(runner, video_path, output_video, output_csv, output_jsonl, warmup=5, max_frames=None):
    ...
    print(
        ...
        f"avg={avg_total_ms:.2f}ms, infer={avg_inference_ms:.2f}ms, fps={bench_fps:.2f}"
    )


def main():
    parser = argparse.ArgumentParser(description="Run PolyLaneNet inference on one video.")
    parser.add_argument("--repo", default="repos/PolyLaneNet", help="Path to PolyLaneNet repo.")
    parser.add_argument("--weights", required=True, help="Path to PolyLaneNet .pt checkpoint.")
    parser.add_argument(
        "--config",
        default="repos/PolyLaneNet/cfgs/tusimple_resnet50.yaml",
        help="PolyLaneNet YAML config. Used for input size/backbone/conf threshold.",
    )
